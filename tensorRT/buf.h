 ...
 */
#ifndef TENSORRT_BUF_H
#define TENSORRT_BUF_H
...
namespace samplesCommon
{
	...
	public:
		...
		//!
		BufferManager(std::shared_ptr<nvinfer1::ICudaEngine> engine, nvinfer1::Dims inDims, nvinfer1::Dims outDims)
			: mEngine(engine)
		{
			// std::cout << mEngine->getNbBindings() << std::endl;
			std::vector<nvinfer1::Dims> dims_{ inDims, outDims };
			for (int i = 0; i < 2; i++)
			{
				auto dim = dims_[i];
				size_t vol = volume(dim);
